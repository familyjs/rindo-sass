 ...
 * @param opts the options provided to the plugin within a Rindo configuration file
...
export function getRenderOptions(
  ...
  // this is a Rindo-specific configuration, and not a part of the Sass API.
      ...
      .map((injectGlobalPath) => {
        if (!path.isAbsolute(injectGlobalPath)) {
          ...
          if (context.sys && typeof context.sys.normalizePath === 'function') {
            // context.sys.normalizePath added in rindo 1.11.0
            injectGlobalPath = context.sys.normalizePath(path.join(context.config.rootDir, injectGlobalPath));
          } else {
            // TODO, eventually remove normalizePath() from @rindo/sass
